...
CUDA_VISIBLE_DEVICES=$CVD MKL_NUM_THREADS=$nt NUMEXPR_NUM_THREADS=$nt OMP_NUM_THREADS=$nt OMP_NUM_THREADS=6 uv run python scripts/evaluate.py --structures_path=$RESULTS_PATH"/result_0/generated_crystals_cif" \
    --relax=True --structure_matcher='disordered'\
    --save_as="$RESULTS_PATH/result_0/metrics.json"
