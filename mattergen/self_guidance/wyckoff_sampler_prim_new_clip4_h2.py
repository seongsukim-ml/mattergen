    ...
    def _score_fn_guidance(
        ...
        def get_conditional_score():
            with torch.set_grad_enabled(True):
                ...
                proj_pos_eps, _ = self._project_to_space_group_epsilon(pred_data)
                pred_data = pred_data.replace(pos=proj_pos_eps)
                ...
                clean_pos_diff = (
                    x.pos - pos_target - torch.round(x.pos - pos_target).detach()
                ).pow(2)
