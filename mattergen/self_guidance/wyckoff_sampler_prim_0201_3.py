    ...
    def _score_fn(
        ...
        def get_conditional_score(model_out):
            ...
            with torch.set_grad_enabled(True):
                ...
                clean_pos = (x.pos.clone() + p_std**2 * pred_data.pos.clone()) / p_mean
                ...
                if self.frac_batch is None:
                    ...
                    if new_fracs_prim.shape[0] != clean_pos.shape[0]:
                        ...
                        # ac_sum = scatter(torch.ones_like(anchors), anchors, dim=0, reduce="sum")
                        # wy_sum = scatter(
                        #     torch.ones_like(wyckoff_batch), wyckoff_batch, dim=0, reduce="sum"
                        # )
                        # idx_sum = scatter(new_fracs_idx * 1.0, wyckoff_batch, dim=0, reduce="sum")
                        # failed = torch.arange(ac_sum.shape[0]).cuda()[(ac_sum - idx_sum) != 0]

                        # import pdb

                        # pdb.set_trace()

                        # Randomize the position for indexing
                ...
                new_fracs_prim_super = new_fracs_prim + torch.round(new_fracs_prim - clean_pos)
                new_fracs_prim_super = new_fracs_prim_super.detach()
                clean_pos_diff = torch.functional.F.mse_loss(
                    clean_pos, new_fracs_prim_super, reduction="none"
                )
                ...
                # prim_clean_lattice_proj_sym = compute_lattice_polar_decomposition(prim_clean_lattice_proj)
                ...
                # ].reshape(-1, 1, 1)
                clean_lattice_diff = (
                    (prim_clean_lattice_proj.detach() - prim_clean_lattice).flatten(-2, -1).abs()
                )
                ...
                # lattice_score2 = -lattice_score * 0.5 * c_std * c_mean
                lattice_score2 = -lattice_score * 1e-7
            ...
            if self.step // 2 in [1, 50, 100, 200, 500, 998]:
            ...
            return pred_data.clone().replace(**scores)
        ...
        if abs(_guidance_scale - 1) < 1e-15:
            ...
            unconditional_score, model_out = get_unconditional_score()
            conditional_score = get_conditional_score(model_out.clone())
