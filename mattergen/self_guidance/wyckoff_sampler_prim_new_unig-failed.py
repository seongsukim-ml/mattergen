    ...
    def _score_fn_guidance(
        ...
        def get_conditional_score():
            with torch.set_grad_enabled(True):
                ...
                proj_pos, proj_cell = self._project_to_space_group(
                    x.clone().replace(cell=prim_clean_lattice, pos=clean_pos)
                )
                ...
                clean_pos_diff = (clean_pos - pos_target - torch.round(clean_pos - pos_target)).pow(
                    2
                )
                ...
                #     1 + pos_mask / (torch.clamp_max((clean_pos_diff.sqrt()).abs(), 1) + 0.05)
                # )
                # if pos_mask.sum() == 0:
                ...
                cell_target = compute_lattice_polar_decomposition(proj_cell)

                clean_lattice_diff = (cell_target - prim_clean_lattice).pow(2)
                ...
                #     1 + cell_mask / (torch.clamp_max((x.cell - cell_target).abs(), 1) + 0.1)
                # )
                # if cell_mask.sum() == 0:
