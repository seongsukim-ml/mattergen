...
class GuidedPredictorCorrector(PredictorCorrector):
        ...
        def get_conditional_score():
            with torch.set_grad_enabled(True):
                ...
                if self.prev_batch is None:
                    proj_pos, proj_cell = self._project_to_space_group(x)
                    proj_pos = proj_pos.detach()
                    proj_cell = proj_cell.detach()
                else:
                    proj_pos = x.proj_pos_target
                    proj_cell = x.proj_cell_target
                ...
                clean_pos_diff = clean_pos_diff * (
                    1 + 3 * pos_mask / (torch.clamp_max((x.pos - pos_target).abs(), 1) + 0.01)
                )
                # if pos_mask.sum() == 0:
                ...
                clean_lattice_diff = clean_lattice_diff * (
                    1 + 3 * cell_mask / (torch.clamp_max((x.cell - cell_target).abs(), 1) + 0.01)
                )
                # if cell_mask.sum() == 0:
    ...
    def _denoise(
        ...
        for i in tqdm(range(self.N), miniters=50, mininterval=5):
            ...
            if self._correctors:
                for _ in range(self._n_steps_corrector):
                    ...
                    if self.prev_batch is not None:
                        proj_pos_target, proj_cell_target = (
                            self.prev_batch.pos,
                            self.prev_batch.cell,
                        )
                    else:
                        proj_pos_target, proj_cell_target = None, None
                    proj_batch = proj_batch.replace(
                        proj_pos_target=proj_pos_target, proj_cell_target=proj_cell_target
                    )
                    proj_mean_batch = proj_mean_batch.replace(
                        proj_pos_target=proj_pos_target, proj_cell_target=proj_cell_target
                    )
            ...
            self.prev_batch = batch.clone().replace(pos=proj_pos, cell=proj_cell)
