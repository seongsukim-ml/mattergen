...
class GuidedPredictorCorrector(PredictorCorrector):
    ...
    def __init__(
        ...
        **kwargs: passed on to parent class constructor.
        ...
        self.guidance_scale_model = guidance_scale_model
    ...
    def _score_fn(
        ...
        if abs(self.guidance_scale_model - 1) < 1e-15:
            return get_conditional_score()
        elif abs(self.guidance_scale_model) < 1e-15:
            ...
            return unconditional_score.replace(
                **{
                    k: torch.lerp(
                        unconditional_score[k], conditional_score[k], self.guidance_scale_model
                    )
    ...
    def _score_fn_guidance(
        ...
        def get_conditional_score():
            with torch.set_grad_enabled(True):
                ...
                clean_pos_diff = clean_pos_diff * (1 + pos_mask * 20)
                # if pos_mask.sum() == 0:
        ...
        if abs(_guidance_scale) < 1e-15:
            ...
            ret = unconditional_score.replace(
                **{
                    k: (
                        torch.lerp(unconditional_score[k], mix_score[k], self.guidance_scale_model)
