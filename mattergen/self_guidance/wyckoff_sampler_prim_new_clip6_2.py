...
class GuidedPredictorCorrector(PredictorCorrector):
        ...
        def get_conditional_score():
            ...
            with torch.set_grad_enabled(True):
                ...
                clean_pos_diff = clean_pos_diff * (
                    1 + pos_mask / (torch.clamp_max((x.pos - pos_target).abs(), 0.95) + 0.05)
                )
                if pos_mask.sum() == 0:
                ...
                # pos_coeff = 0.03 / p_std**2
                pos_coeff = 0.03 / p_std
                # pos_coeff = 1
    ...
    def _denoise(
        ...
        sym_atom = proj_batch.atomic_numbers[self.anchors]
        proj_batch = proj_batch.replace(atomic_numbers=sym_atom)
        proj_mean_batch = proj_mean_batch.replace(atomic_numbers=sym_atom)
