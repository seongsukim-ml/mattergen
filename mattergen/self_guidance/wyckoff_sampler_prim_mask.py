...
class GuidedPredictorCorrector(PredictorCorrector):
    ...
    def _score_fn_guidance(
        ...
        def get_conditional_score():
            x_no_conditioning = self._remove_conditioning_fn(x)
            x_mask = x_no_conditioning.clone()
            pred_data = self.diffusion_module.model(x_mask, t)
            ...
            if (self.step + 1) in [
                ...
                print(f"Step {self.step+1}~~~~~~~~~~~~~~~~~~~~~~")
                print(f"pos: {x.pos}")
                # print(f"pos_proj: {proj_pos}")
                # print(f"pos_mask: {pos_mask}")
                print(f"Step {self.step+1}~~~~~~~~~~~~~~~~~~~~~~")
                print(f"cell: {x.cell}")
                # print(f"cell_proj: {proj_cell}")
    ...
    def _denoise(
        ...
        for i in tqdm(range(self.N), miniters=50, mininterval=5):
            ...
            if self._correctors:
                for _ in range(self._n_steps_corrector):
                    ...
                    proj_batch = proj_batch.replace(cell=proj_cell)
                    proj_mean_batch = proj_mean_batch.replace(
                        proj_pos=proj_pos, proj_cell=proj_cell
                    )
                    proj_mean_batch = proj_mean_batch.replace(cell=proj_cell)

                    proj_pos_batch, _ = self._project_to_space_group(proj_batch)
                    proj_batch = proj_batch.replace(pos=proj_pos_batch)
                    proj_mean_batch = proj_mean_batch.replace(pos=proj_pos_batch)
                    proj_cell = compute_lattice_polar_decomposition(proj_cell)
                    proj_batch = proj_batch.replace(cell=proj_cell)
                    proj_mean_batch = proj_mean_batch.replace(cell=proj_cell)
        ...
        final_proj_batch = proj_mean_batch.clone()
